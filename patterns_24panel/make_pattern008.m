...
Pats = zeros(5, 24*8, pattern.x_num, pattern.y_num);
...
InitPat = [ones(5,142).*(2^pattern.gs_val-1) ones(5,4).*(2^pattern.gs_val-1) ones(5,46).*(2^pattern.gs_val-1)];
Pats(:,:,1,1) = InitPat;
...
directory_name = 'C:\Users\maimonlab\Desktop\Vikram\current code for VR\patterns_24panel'
str = [directory_name '\Pattern_008'] 	% name must begin with ‘Pattern_’
save(str, 'pattern');
