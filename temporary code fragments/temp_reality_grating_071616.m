...
% ----- ------ --------- ------------------------------
% 1     ni     Dev4      National Instruments PCIe-6351
% 2     ni     Dev5      National Instruments PCIe-6351

% Dev 4 is used in this Matlab code
% Dev 5 is for streaming in WinEDR

function temp_reality_grating_071616(file_name, duration, world_num, visual_display, dump_mode, pid_mode, angle_mode)
...
In0 = addAnalogInputChannel(s,'Dev4', 1, 'Voltage');
In1 = addAnalogInputChannel(s,'Dev4', 3, 'Voltage');
In2 = addAnalogInputChannel(s,'Dev4', 6, 'Voltage');
In3 = addAnalogInputChannel(s,'Dev4', 10, 'Voltage');

% Out0 is duty cycle output
% Out1 is camera trigger
% Out0 = addAnalogOutputChannel(s2,'Dev4', 0, 'Voltage');
% Out1 = addAnalogOutputChannel(s2,'Dev4', 1, 'Voltage');

% These channels are not currently being used
% addCounterOutputChannel(s,'Dev4', 0, 'PulseGeneration');
% addCounterOutputChannel(s,'Dev4', 1, 'PulseGeneration');
...
if(visual_display > 0)
    ...
    filep = load(['C:\Users\maimonlab\Desktop\Vikram\beta code for VR2\patterns_24panel\' visdisplay_string],'pattern');
...
tmr_worldchange1 = timer('ExecutionMode', 'FixedRate', ...
    'Period',60, ...
    'TimerFcn', {@world_change_1});

tmr_worldchange2 = timer('ExecutionMode',  'FixedRate', ...
    'Period',60, ...
    'StartDelay',50, ...
...
dutytext = uicontrol('Style','text',...
    'Position',[10 200 125 100],...
    'String','Duty');

% if(visual_display >0)
%     % This button is to offset visual display (only avaible in closed loop
%     % mode)
...
%         'Position',[20 140 80 55],...
%         'String','Offset Display to Absolute position in world (0 to 360)');
% end
...
% Patterns to run in openloop
pt_run = [28:33, 28:33];
pt_gain = [ones(1,6), -1.*ones(1,6)];

[tmp_a, tmp_b] = sort(rand(1,12));
pt_runt = [pt_run(tmp_b),pt_run(tmp_b),pt_run(tmp_b)] ;
pt_gaint = [pt_gain(tmp_b),pt_gain(tmp_b),pt_gain(tmp_b)];

pt_run = 9.*ones(1,48);
pt_gain = ones(1,48);
pt_run(2:2:72) = pt_runt;
pt_gain(2:2:72) = pt_gaint;
pt_run(1:2:71) = pt_runt;
pt_gain(1:2:71) = pt_gaint;
...
if(~s.IsRunning)
    ...
    function world_change_1(~, ~)
        ...
        if(cnt > 72)
            ...
            trial_id = pt_run(cnt).*pt_gain(cnt).*flip;
            
            Panel_com('stop');
            Panel_com('set_pattern_id', pt_run(cnt));
            Panel_com('set_ao', [3 0]);
            Panel_com('send_gain_bias', [pt_gain(cnt).*12.*flip, 0, 0, 0]);
            Panel_com('start')
        end
    ...
    function world_change_2(~, ~)
       cnt = cnt+1;
        ...
        if(cnt > 72)
            ...
            Panel_com('set_pattern_id', pt_run(cnt));
            Panel_com('set_ao', [3 0]);
            Panel_com('send_gain_bias', [pt_gain(cnt).*128.*flip, 0, 0, 0]);
            Panel_com('start')
        end
    ...
    function timer_for_realtimeplot(~, ~)
        ...
        set(dutytext,'String',['Duty = ' num2str(duty) ', FlyTemp = ' num2str(fly_temperature) ', TimeElap = ' num2str(time_elapsed)]);
    ...
    function doIntegration(~, event)
        ...
        if(dump_mode)
            ...
            if(head_angle(end) ~= 0)
                ...
                framedump(filep.pattern.Pats(:,:,send_pat), filep.pattern.Panel_map, 24, 5);
            else
                send_pat = 1;
                framedump(filep.pattern.Pats(:,:,send_pat), filep.pattern.Panel_map, 24, 5);
            end
        ...
        elp = toc; tic;
        write_to_file = [x; y; head_angle(end); event.Data(end,1); event.Data(end,3); duty; current_spacemap; pattern_xnum; visual_display; tgain; rgain; current_openloop_mode; trial_id; fly_temperature; desired_dutyortemp; send_pat; elp];
        fprintf(write_file,'%3f %3f %3f %3f %3f %12.8f %3f %3f %3f %3f %3f %3f %3f %3f %3f %3f %3f\r\n',write_to_file);
...
%         if(current_visualoffset > 360 || current_visualoffset < 0)
%             stop(s);
%         end
%         visual_display_cmap = zeros(256,3);
%         visual_display_cmap(:,2) = 0:1/256:1-1/256;
%         visdisplay_string = pad_zeros_for_pattern(visual_display);
%         filep = load(['C:\Users\maimonlab\Desktop\Vikram\beta code for VR2\patterns_24panel\' visdisplay_string],'pattern');
%         [rows, cols, npats] = size(filep.pattern.Pats);
%         gs_val = filep.pattern.gs_val;
%         [sx,sy,sz] = cylinder(rows,cols);
%         tmp_pat = [];
%         offset_in_frames = round(mod(current_visualoffset./360*npats+1,npats));
%         Panel_com('stop');
%
%         if(current_visualoffset <=180)
%             Panel_com('send_gain_bias', [1, round((current_visualoffset+180)/360*200-100), 0, 0]);
%         end
%
%         if(current_visualoffset >180)
%             Panel_com('send_gain_bias', [1, round(-1*(360-current_visualoffset)/360*200), 0, 0]);
%         end
%
%         Panel_com('start');
%         tmp_pat(:,:,1) = zeros(rows,cols);
%         tmp_pat(:,:,2) = flipud(filep.pattern.Pats(:,:,offset_in_frames));
%         tmp_pat(:,:,3) = zeros(rows,cols);
