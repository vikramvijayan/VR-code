...
function temp_reality_jump_forshi(file_name, duration, world_num, visual_dis, dump_mode, pid_mode, angle_mode, jump_mode)

global visual_display;
global display_change;

display_change = 0;
visual_display = visual_dis;
...
% Timer that can change the visuaul display
 tmr_displaychange1 = timer('ExecutionMode', 'SingleShot', ...
     'StartDelay',60*50, ...
     'TimerFcn', {@display_change_1});
...
if(~s.IsRunning)
    ...
    if(pid_mode)
        ...
        delete(vid_temp);
          ...
          stop(tmr_displaychange1);
      delete(tmr_displaychange1);
...
%             end
        ...
        if(dump_mode)
            ...
            if(head_angle(end) ~= 0)
                ...
                framedump(filep.pattern.Pats(:,:,send_pat), filep.pattern.Panel_map, 24, 5, round(send_pat/numpats*32767), round((jump+man_jump+display_change)/10*32767));
            else
                send_pat = 1;
                framedump(filep.pattern.Pats(:,:,send_pat), filep.pattern.Panel_map, 24, 5, round(send_pat/numpats*32767), round((jump+man_jump+display_change)/10*32767));
            end
