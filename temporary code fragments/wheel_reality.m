...
% 8) learning_mode (1:fwd, 2:back, laser off for specified motion)
...
function wheel_reality(file_name, duration, world_num, visual_display,...
    dump_mode, pid_mode, angle_mode, learning_mode)
...
%for 2 Hz integration. used for learning
global y_curr;
global y_last;
global queue;
queue = [];
...
In2 = addAnalogInputChannel(s,'Dev6', 7, 'Voltage');
...
tmr_2s = timer('ExecutionMode', 'SingleShot', ...
    'StartDelay',60, ...
    'TimerFcn', {@timer_for_60slaseroff});
...
% This button is to turn the laser off for 1 min
btn4 = uicontrol('Style', 'pushbutton', 'String', '60 s laser OFF',...
    'Position', [20 650 80 20],...
    'Callback', @bpress4);
...
if(~s.IsRunning)
    ...
    function timer_for_60slaseroff(~, ~)
        current_spacemap = world_num;
        space_map = get_world(world_num, world_size);
        %         delete(draw_map);
        %         set(FigA,'CurrentAxes',plot_axes_hnd);
    ...
    function doIntegration(~, event)
        ...
        if(isempty(yp))
            y_pos1 = diff(unwrap([pi*(event.Data(:,3)-1)./1.5]))./(2*pi);
                        
        else
            y_pos1 = diff(unwrap(pi*[(yp-1); (event.Data(:,3)-1)]./1.5))./(2*pi);
        ...
        if(learning_mode)
            ...
            if(length(queue) == 5)
                ...
                if(learning_mode == 1)
                    if isempty(y_last)
                        y_last = y_curr+1;   %higher to not heat immediately
                    end
                    if((y_last - y_curr) < -0.01)
                        duty = 0;
                    else
                        duty = 0.08;
                    end
                ...
                elseif(learning_mode == 2)
                    if isempty(y_last)
                        y_last = y_curr-1;   %lower to not heat immediately
                    end
                    if((y_last - y_curr) > 0.01)
                        duty = 0;
                    else
...
%             prev_duty = duty;
%         else
%             %duty = desired_dutyortemp;       commented out to enable
%             %learning mode
...
%         end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %         %set PWM depending on elapsed time for 5-15-5-15-5-15 experiment
        %         %can't use world change button during laser off periods!
        %
        %         if(time_elapsed < 301 || ...
        %             time_elapsed > 1200 && time_elapsed < 1501 || ...
        %             time_elapsed > 2400 && time_elapsed < 2701)
        %
        %             duty = 0;
        %
        %         end
        ...
        if(dump_mode)
            ...
            send_pat = numpats+1-ceil(mod(y,1)*numpats);
            if(send_pat ==  (numpats+1))
                send_pat =  1;
            end
            
            framedump(filep.pattern.Pats(:,:,send_pat), filep.pattern.Panel_map, 24, 5, round(send_pat/numpats*32767), round(send_pat/numpats*32767));
    ...
    function bpress4(hObject, callbackdata)
        current_spacemap = 1;
        space_map = get_world(current_spacemap, world_size);
        %         delete(draw_map);
        %         set(FigA,'CurrentAxes',plot_axes_hnd);
